...
if(!is_numeric($_GET['id'])){ DrawFooter(); exit(); }
	...
	$out = SqlObjQuery("SELECT `planetname` FROM `planets` WHERE `id` = {$_GET['id']};");
	echo "<form action='planet.php?id={$_GET['id']}' method='POST'><input id='pname' name='pname' value=\"".$out->planetname."\"/><input id='SEND' type='submit' value='&auml;ndern'></form>";
...
<?php
	...
	if($userdata->id == $_SESSION['userid']){
		echo "</center><br/><a href=\"planet.php?id={$planetdata->id}\">Zu Planet wechseln</a><br/><center>";
	}
